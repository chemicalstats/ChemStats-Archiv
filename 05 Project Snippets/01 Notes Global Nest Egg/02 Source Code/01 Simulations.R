...
msci_data <- list(
  ...
  read.csv("./01 Source Data/Bundesbank ST0101.csv", skip = 8, sep = ";") %>%
    select(date = 1, ftgr = 2) %>%
    mutate(
      date = as.Date(date, format = "%Y-%m-%d"),
      ftgr = as.numeric(gsub(",", ".", na_if(ftgr, ".")))),
  read.csv("./01 Source Data/FRED Euro Interbank Offered Rate.csv", na.strings = ".") %>%
    rename(date = 1, eonia = 2) %>%
    mutate(date = as.Date(date)),
  read.csv("./01 Source Data/FRED Euro Short Term Rate.csv", na.strings = ".") %>%
    rename(date = 1, ester = 2) %>%
    mutate(date = as.Date(date)),
  #---> 5. Integration des MSCI All-Country World in den USD-Varianten Price, Gross Total und Net Total Return:
  list(
   data.frame(date = seq.Date(from = as.Date(start_date),
                             to = as.Date(paste0(year(Sys.Date()), "-12-31")),
                             by = "days")),
  ...
  #---> 6. Integration des Leverage MSCI All-Country World Index für den USD-Gross Total Return:
  ...
  #---> 7. Berechnung der Indexrenditen und Erweiterung der Simulationen:
  mutate(
...
msci_model <- msci_data %>%
  ...
  select(date, days, us_rate, eu_rate, eucr, sim_price, sim_price_return, 
...
msci_us_final <- msci_model %>%
  mutate(
    ...
    select(date, days, us_rate, eucr, starts_with("sim_"), -ends_with("_return")) %>%
    mutate(across(-c(date, days, us_rate, eucr), ~ .x/first(.x) * 100))

msci_eu_final <- msci_model %>%
  mutate(
    sim_price = sim_price * eucr,
    sim_gdtr_l1 = sim_gdtr_l1 * eucr,
    sim_gdtr_l1_return = sim_gdtr_l1 / lag(sim_gdtr_l1),
    sim_gdtr_l2_return = (2 * sim_gdtr_l1_return + (1 - 2) * (eu_rate/100) * (1/days)) - 1 - (0.0015/100),
    sim_gdtr_l2 = round(Reduce(function(values, rates){values * rates},
                        lead(sim_gdtr_l2_return), init = 100, accumulate = TRUE)[-nrow(.)], digits = 5),
    sim_gdtr_l2 = case_when(row_number() == n() ~ round(lag(sim_gdtr_l2) * sim_gdtr_l2_return, digits = 2),
                            TRUE ~ sim_gdtr_l2),
    sim_ndtr_l1 = sim_ndtr_l1 * eucr,
    sim_ndtr_l1_return = sim_ndtr_l1 / lag(sim_ndtr_l1),
    sim_ndtr_l2_return = (2 * sim_ndtr_l1_return + (1 - 2) * (eu_rate/100) * (1/days)) - 1 - (0.0015/100),
    sim_ndtr_l2 = round(Reduce(function(values, rates){values * rates},
                        lead(sim_ndtr_l2_return), init = 100, accumulate = TRUE)[-nrow(.)], digits = 5),
    sim_ndtr_l2 = case_when(row_number() == n() ~ round(lag(sim_ndtr_l2) * sim_ndtr_l2_return, digits = 2),
                            TRUE ~ sim_ndtr_l2)) %>%
  select(date, days, eu_rate, eucr, starts_with("sim_"), -ends_with("_return")) %>%
  mutate(across(-c(date, days, eu_rate, eucr), ~ .x/first(.x) * 100))
...
msci_eu_final %>%
  ...
  layout(
    showlegend = TRUE, legend = list(orientation = 'h'), 
    xaxis = list(title = "Time"),
    yaxis = list(title = "Log. Index Value (EUR)", range = c(0, 5)),
    title = "MSCI All-Country World",
    margin = list(t = 50))
...
msci_us_final %>%
  ...
  write.csv("./All-Country World USD Index Data.csv", row.names = FALSE)
...
msci_eu_final %>%
  mutate(
    ...
    letf_acwi_return = sim_ndtr_l2 / lag(sim_ndtr_l2),
    letf_acwi_return = letf_acwi_return - (((letf_acwi_td + letf_acwi_ter)/100 + 1)^(1/days) - 1),
    letf_acwi_eur_sim = Reduce(function(values, rates){values*rates},
                        lead(letf_acwi_return), init = 100, accumulate = TRUE)[-nrow(.)],
    letf_acwi_eur_sim = case_when(row_number() == n() ~ round(lag(letf_acwi_eur_sim)*letf_acwi_return, digits = 2),
                                  TRUE ~ letf_acwi_eur_sim),
    letf_acwi_eur_sim = letf_acwi_eur_sim/first(letf_acwi_eur_sim) * 100) %>%
  select(-c(eucr, days, letf_acwi_td, letf_acwi_ter, letf_acwi_return)) %>%
  write.csv("./All-Country World EUR Index Data.csv", row.names = FALSE)
...
calc_vev <- function(r, N = 252){
  ...
  vev <- if(is.na(inner) || inner < 0){sigma * sqrt(252)} else {(sqrt(inner) - 1.96) / sqrt(T_years)}
...
#---> 1. Anwendung auf USD-Simulationsdaten
msci_input <- read.csv("./All-Country World USD Index Data.csv") %>%
  mutate(date = as.Date(date)) %>%
  filter(!weekdays(date) %in% c("Saturday", "Sunday")) %>%
  select(date, price = letf_acwi_eur_sim) %>%
  filter(!is.na(price))
...
mrm_output %>%
  ...
  add_lines(y = ~vev * 100,  name = "VEV (%)", line = list(color = "steelblue", width = 1.5), yaxis = "y1") %>%
  add_lines(y = ~mrm_class, name = "MRM-Klasse", line = list(color = "firebrick", width = 2), yaxis = "y2") %>%
   layout(
    title = "Market Risk Measure (Daily Data): Leveraged MSCI ACWI Daily Swap Proxy\nAssumed Leverage Factor: x2 - Total Expense Ration: 0.70% - Additional Costs: 0.50%",
...
#---> 2. Anwendung auf EUR-Simulationsdaten
msci_input <- read.csv("./All-Country World EUR Index Data.csv") %>%
  mutate(date = as.Date(date)) %>%
  filter(!weekdays(date) %in% c("Saturday", "Sunday")) %>%
  select(date, price = letf_acwi_eur_sim) %>%
  filter(!is.na(price))
...
mrm_output %>%
  ...
  add_lines(y = ~mrm_class, name = "MRM-Klasse", line = list(color = "firebrick", width = 2), yaxis = "y2") %>%
   layout(
    title = "Market Risk Measure (Daily Data): Leveraged MSCI ACWI Daily Swap Proxy\nAssumed Leverage Factor: x2 - Total Expense Ration: 0.45% - Additional Costs: 0.50%",
    xaxis  = list(title = "", nticks = 10),
    yaxis  = list(title = "VEV (%)", side = "left", showgrid = FALSE),
    yaxis2 = list(title = "MRM Class", side = "right",
                  overlaying = "y", tickvals = 1:7, range = c(0.5, 7.5), showgrid = FALSE),
...
########################################################################################################################
vev_of_leverage <- function(L, sigma, skew, kurt, N = 1, ter, td, fin_rate){
  sigma_L <- L * sigma
  cost_daily <- (ter/100 + td/100) / 252 + (fin_rate * max(L - 1, 0)) / 360
...
solve_leverage <- function(target_vev, sigma, skew, kurt, N, ter, td, fin_rate, L_range = c(0.1, 10)){
...
leverage_for_mrm5 <- function(data, window, N, step_days = 21, ter, td, rate = rate){
  daily <- data %>%
    arrange(date) %>%
    filter(!is.na(price), price > 0) %>%
    mutate(log_ret = log(price / lag(price))) %>%
  ...
  map_dfr(calc_indices, function(i){
    ...
    fin_rate_window <- mean(daily[[rate]][(i - window + 1):i], na.rm = TRUE) / 100
...
#---> 1. Anwendung auf USD-Simulationsdaten
base_usd_input <- list(
  read.csv("./All-Country World USD Index Data.csv") %>%
    mutate(date = as.Date(date)),
  read.xlsx("./01 Source Data/FRED Effective Fedral Funds Rate RIFSPFFNB.xlsx", sheet = 2) %>%
    rename(date = observation_date, effr = RIFSPFFNB) %>%
    mutate(date = as.Date(date, origin = "1899-12-30")),
...
lev_corridor <- leverage_for_mrm5(base_usd_input, window = 1260, N = 1, step_days = 1, ter = 0.7, td = 0.5, rate = "us_rate")

lev_corridor %>%
  plot_ly(x = ~date) %>%
  add_ribbons(ymin = ~0, ymax = ~L_max_rk5, name = "Zulässiger Hebel", fillcolor = "rgba(70, 130, 180, 0.25)", line = list(color = "transparent")) %>%
  add_lines(y = ~L_max_rk5, name = "Max. L (VEV = 30%)", line = list(color = "firebrick", width = 1)) %>%
  add_segments(x = ~min(date), xend = ~max(date), y = 2, yend = 2, line = list(color = "gray40", dash = "dash", width = 1), name = "Konstanter Hebel 2x") %>%
  layout(
...
#---> 2. Anwendung auf EUR-Simulationsdaten
base_eur_input <- list(
  read.csv("./All-Country World EUR Index Data.csv") %>%
    mutate(date = as.Date(date)),
  read.csv("./01 Source Data/Bundesbank ST0101.csv", skip = 8, sep = ";") %>%
    select(date = 1, ftgr = 2) %>%
    mutate(
  ...
  mutate(
    ...
    date = as.Date(date)) %>%
  filter(!weekdays(date) %in% c("Saturday", "Sunday")) %>%
  select(date, price = sim_ndtr_l1, eu_rate) %>%
  filter(!is.na(price))

lev_corridor <- leverage_for_mrm5(base_eur_input, window = 1260, N = 1, step_days = 1, ter = 0.45, td = 0.50, rate = "eu_rate")
...
########################################################################################################################
# Berechnung der Select-Indizes — MSCI-Adjustment-Factor
########################################################################################################################
# Formel (Parameter Sheet, Mai 2026):
#   R_L(t) = L' * R(t) + (1 - L') * r_f * (T/360),   L' = L * AF(t)
#   AF(t)  = [LI(t-2)/EI(t-2)] / [LI(t-1)/EI(t-1)]   (= 1, solange Historie fehlt)
#   LI(t)  = LI(t-1) * (1 + R_L(t))
leveraged_select <- function(base, rf, tdays, g = 2, start = 100, fee = 0, g_min = 1.5, g_max = 2, adjust = TRUE, detail = FALSE){
  n <- length(base)
  li <- af_vec <- gadj_vec <- rep(NA_real_, n)
  R  <- c(NA, base[-1] / base[-n] - 1)
  li[1] <- start
  for (t in 2:n) {
    if (adjust && t > 2 &&
        all(is.finite(c(li[t-1], li[t-2], base[t-1], base[t-2]))) && base[t-1] != 0 && base[t-2] != 0){
      af <- (li[t-2] / base[t-2]) / (li[t-1] / base[t-1])
    } else af <- 1
    g_adj <- min(max(g * af, g_min), g_max)
    af_vec[t] <- af; gadj_vec[t] <- g_adj
    rl <- g_adj * R[t] + (1 - g_adj) * (rf[t]/100) * (tdays[t]/360)
    li[t] <- li[t-1] * (1 + rl - fee)
  }
  if (detail) tibble(li = li, af = af_vec, g_adj = gadj_vec) else li
}

#---> 1. Simulation der Select-Indizes
msci_select <- list(
  msci_us_final %>%
    arrange(date) %>%
    mutate(
      T_cal = as.numeric(date - lag(date)),
      T_cal = ifelse(is.na(T_cal) | T_cal <= 0, 1, T_cal),
      sim_ndtr_l2_select = leveraged_select(base = sim_ndtr_l1, rf = us_rate, tdays = T_cal, g = 2, start = 100, fee = 0, adjust = TRUE)) %>%
    select(date, days, eucr, sim_ndtr_l1, sim_ndtr_l2, sim_ndtr_l2_select) %>%
    mutate(across(-c(date, days, eucr), ~ .x / first(.x) * 100)) %>%
    rename_with(~ paste0(.x, "_usd"), -c(date, days, eucr)),
  msci_eu_final %>%
    arrange(date) %>%
    mutate(
      T_cal  = as.numeric(date - lag(date)),
      T_cal  = ifelse(is.na(T_cal) | T_cal <= 0, 1, T_cal),
      sim_ndtr_l2_select = leveraged_select(base = sim_ndtr_l1, rf = eu_rate, tdays = T_cal, g = 2, start = 100, fee = 0, adjust = TRUE)) %>%
    select(date, days, eucr, sim_ndtr_l1, sim_ndtr_l2, sim_ndtr_l2_select) %>%
    mutate(across(-c(date, days, eucr), ~ .x / first(.x) * 100)) %>%
    rename_with(~ paste0(.x, "_eur"), -c(date, days, eucr))) %>%
  Reduce(function(x, y) left_join(x, y, by = c("date", "days", "eucr")), .)

msci_select %>%
  mutate(across(where(is.numeric), function(x){x/first(x)})) %>%
  plot_ly(type = "scatter", mode = "lines") %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l1_usd), name = "Net Total Return x1 USD", line = list(width = 1)) %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l1_eur), name = "Net Total Return x1 EUR", line = list(width = 1)) %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l2_usd), name = "Net Total Return x2 USD", line = list(width = 1)) %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l2_eur), name = "Net Total Return x2 EUR", line = list(width = 1)) %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l2_select_usd), name = "Net Total Return Select x2 USD", line = list(width = 1)) %>%
  add_trace(x = ~date, y = ~log(sim_ndtr_l2_select_eur), name = "Net Total Return Select x2 EUR", line = list(width = 1)) %>%
  layout(
    showlegend = TRUE, legend = list(orientation = 'h'), 
    xaxis = list(title = "Time"),
    yaxis = list(title = "Log. Index Value (USD & EUR)", range = c(0, 5)),
    title = "MSCI All-Country World Indices Leverage Base vs. Select",
    margin = list(t = 50))
...
msci_select %>%
  mutate(
    letf_acwi_td = 0.5,
    letf_acwi_ter = 0.45,
    letf_acwi_base_return = sim_ndtr_l2_eur / lag(sim_ndtr_l2_eur),
    letf_acwi_base_return = letf_acwi_base_return - (((letf_acwi_td + letf_acwi_ter)/100 + 1)^(1/days) - 1),
    letf_acwi_base_eur_sim = Reduce(function(values, rates){values*rates}, lead(letf_acwi_base_return), init = 100, accumulate = TRUE)[-nrow(.)],
    letf_acwi_base_eur_sim = case_when(row_number() == n() ~ round(lag(letf_acwi_base_eur_sim)*letf_acwi_base_return, digits = 2),
                                  TRUE ~ letf_acwi_base_eur_sim),
    letf_acwi_base_eur_sim = letf_acwi_base_eur_sim/first(letf_acwi_base_eur_sim) * 100,
    letf_acwi_select_return = sim_ndtr_l2_select_eur / lag(sim_ndtr_l2_select_eur),
    letf_acwi_select_return = letf_acwi_select_return - (((letf_acwi_td + letf_acwi_ter)/100 + 1)^(1/days) - 1),
    letf_acwi_select_eur_sim = Reduce(function(values, rates){values*rates}, lead(letf_acwi_select_return), init = 100, accumulate = TRUE)[-nrow(.)],
    letf_acwi_select_eur_sim = case_when(row_number() == n() ~ round(lag(letf_acwi_select_eur_sim)*letf_acwi_select_return, digits = 2),
                                  TRUE ~ letf_acwi_select_eur_sim),
    letf_acwi_select_eur_sim = letf_acwi_select_eur_sim/first(letf_acwi_select_eur_sim) * 100) %>%
  select(-c(eucr, days, 
    sim_ndtr_l1_eur, sim_ndtr_l2_eur, sim_ndtr_l2_select_eur, 
    sim_ndtr_l1_usd, sim_ndtr_l2_usd, sim_ndtr_l2_select_usd, 
    letf_acwi_td, letf_acwi_ter, letf_acwi_base_return, letf_acwi_select_return)) %>%
  write.csv("./All-Country World EUR Select Index Data.csv", row.names = FALSE)


########################################################################################################################
# Diagnose: Effektiver Tageshebel L' des Select-Index
########################################################################################################################
lev_path <- list(
  msci_us_final %>%
    arrange(date) %>%
    ...
    {tibble(
      date = .$date,
      r_parent = .$sim_ndtr_l1 / lag(.$sim_ndtr_l1) - 1,
      af = leveraged_select(.$sim_ndtr_l1, .$us_rate, .$T_cal, g_min = 0, g_max = Inf, detail = TRUE)$af,
      g_uncapped  = leveraged_select(.$sim_ndtr_l1, .$us_rate, .$T_cal, g_min = 0, g_max = Inf, detail = TRUE)$g_adj,
      g_capped = leveraged_select(.$sim_ndtr_l1, .$us_rate, .$T_cal, g_min = 1.5, g_max = 2, detail = TRUE)$g_adj,
      ccy = "USD")},
  msci_eu_final %>%
    arrange(date) %>%
    mutate(
    ...
    {tibble(
      date = .$date,
      r_parent = .$sim_ndtr_l1 / lag(.$sim_ndtr_l1) - 1,
      af = leveraged_select(.$sim_ndtr_l1, .$eu_rate, .$T_cal, g_min = 0, g_max = Inf, detail = TRUE)$af,
      g_uncapped = leveraged_select(.$sim_ndtr_l1, .$eu_rate, .$T_cal, g_min = 0, g_max = Inf, detail = TRUE)$g_adj,
      g_capped = leveraged_select(.$sim_ndtr_l1, .$eu_rate, .$T_cal, g_min = 1.5, g_max = 2, detail = TRUE)$g_adj,
      ccy = "EUR")}) %>%
  bind_rows() %>%
  mutate(r_vortag = lag(r_parent), .by = ccy) %>%
  filter(!is.na(g_uncapped))

subplot(
  lev_path %>%
    filter(ccy == "EUR") %>%
    plot_ly(x = ~date) %>%
    add_lines(y = ~g_uncapped, name = "Ohne L-Cap", line = list(color = "lightsteelblue", width = 0.5)) %>%
    add_lines(y = ~g_capped, name = "L-Cap im Spektrum von 1.5x bis 2x", line = list(color = "steelblue", width = 0.7)) %>%
    layout(
      title = "Effektiver Tageshebel L' des MSCI ACWI Leveraged 2x Select (EUR)",
      xaxis = list(title = ""),
      yaxis = list(title = "L'", range = c(1.75, 2.30)),
      legend = list(orientation = "h"),
      margin = list(t = 50)), 
  lev_path %>%
    filter(ccy == "EUR") %>%
    plot_ly() %>%
    add_histogram(x = ~g_uncapped, nbinsx = 100, name = "Ohne L-Cap", marker = list(color = "lightsteelblue")) %>%
    add_histogram(x = ~g_capped, nbinsx = 100, name = "L-Cap im Spektrum von 1.5x bis 2x", marker = list(color = "steelblue", opacity = 0.6)) %>%
    layout(
      barmode = "overlay",
      title = "Zeitverlauf und Verteilung des Tageshebels L'",
      xaxis = list(title = "L'"),
      yaxis = list(title = "Anzahl Handelstage"),
      legend = list(orientation = "h"),
      margin = list(t = 50)),
